...
class Agent(object):
    def __init__(self):
        ...
        self.D = 40  # input dimensionality: 100x100 grid
        self.prev_x = None
        self.model = {}
        self.init_model()
        self.name = "6ComboDestroyer"
        self.rewards = []
        self.model_file = "fourty.p"
    ...
    def train(self,reward):
        ...
        if self.episode_number % 500 == 0:
            ...
            y = y[start:]
            y = y[0:(y.size - y.size % grouping)]
            y = np.mean(y.reshape(-1, grouping), axis=1)

            x = np.linspace(start,start+y.size*grouping,y.size)
            ...
            plt.subplot(2, 1, 1)
            plt.title("Running reward over training episodes")
            plt.plot(x, y, label='Original data', markersize=3)
            plt.plot(x, m*x + b, 'r', label='Fitted line')
            plt.legend()

            plt.subplot(2, 1, 2)
            plt.title("Running reward histogram")
            plt.hist(y)

            plt.savefig('./running_rewards_plot.png')
    ...
    def get_action(self, observation):
        ...
        # if self.first:
        #     pickle.dump(self.model, open(self.model_file, 'wb'))
        #     prediction = np.array([np.random.uniform(0, 200), np.random.uniform(0, 200), np.random.uniform(0, 200), np.random.uniform(0, 200)])
        ...
        self.screens = np.delete(self.screens, [0,1,2,3], axis=0)
        self.screens = np.append(self.screens, prediction, axis=0)
